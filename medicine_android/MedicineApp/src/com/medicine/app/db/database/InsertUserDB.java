...
public class InsertUserDB implements CommonConst{
	...
	public void insertUserInfo(UserInfoBean info) {
		...
		if (db.isOpen()) {
			db.execSQL(
					"insert into "+CommonConst.TABLE_NAME +" (ID,ICODE,StartTime,Name,Sex,BornYearMonth,llhistory,llhistorydetails) values (?,?,?,?,?,?,?,?)",
					new String[] { info.getID(), info.getICODE(),
							info.getStartTime(), info.getName(),
							info.getSex(), info.getBornYearMonth(),
							info.getLlhistory(),info.getLlhistorydetails() });
			db.close();
		}
	}
		...
		if (db.isOpen()) {
			...
			if (mCursor != null) {
				while (mCursor.moveToNext()) {
					...
					String llhistory = mCursor.getString(mCursor.getColumnIndex("llhistory"));
					String llhistorydetails = mCursor.getString(mCursor.getColumnIndex("llhistorydetails"));
					list.add(new UserInfoBean(id, idCode, startTime, name, sex, bornYearMonth, llhistory, llhistorydetails));
				}
				mCursor.close();
			}
			db.close();
